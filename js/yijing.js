...
const YiJing = {
    ...
    renderResult(result) {
        ...
        let html = `
            ...
            ${this.renderLines(hexagram.lines, isEn, isJa)}
            <div class="analysis-card">
                <h4>${isJa ? '卦象の意味' : isEn ? 'Hexagram Meaning' : '卦象含义'}</h4>
                <p>${hexagram.meaning}</p>
            </div>
            <div class="analysis-card">
                <h4>${isJa ? '行動アドバイス' : isEn ? 'Guidance' : '行事建议'}</h4>
                <p>${hexagram.advice}</p>
            </div>
            <div class="analysis-card">
                <h4>${isJa ? '上卦と下卦' : isEn ? 'Upper & Lower Trigrams' : '上卦与下卦'}</h4>
                <p>${isJa ? '上卦' : isEn ? 'Upper' : '上卦'}：${hexagram.upper?.name || '乾'}（${hexagram.upper?.nature || '天'}）</p>
                <p>${isJa ? '下卦' : isEn ? 'Lower' : '下卦'}：${hexagram.lower?.name || '坤'}（${hexagram.lower?.nature || '地'}）</p>
            </div>
            <div class="analysis-card disclaimer-card">
                <p class="disclaimer-note" style="font-size: 0.85rem; color: #888;">
                    ${isJa ? '⚠️ 易経占いは参考程度にね、重大な決断には使わないでよ～' : isEn ? '⚠️ Yi Jing divination is for guidance only, not for major decisions~' : '⚠️ 易经占卜仅供参考，不作为重大决策依据～'}
                </p>
            </div>
        `;
     ...
     * 渲染卦象线条（支持三语）
     */
    renderLines(lines, isEn = false, isJa = false) {
        let html = '<div class="hexagram-lines">';
        const positionsZh = ['初爻', '二爻', '三爻', '四爻', '五爻', '上爻'];
        const positionsEn = ['1st', '2nd', '3rd', '4th', '5th', '6th'];
        const positionsJa = ['初爻', '二爻', '三爻', '四爻', '五爻', '上爻'];

        // 从下到上显示
        for (let i = 5; i >= 0; i--) {
            const isYang = lines[i] === '1';
            const position = isJa ? positionsJa[i] : isEn ? positionsEn[i] : positionsZh[i];
